...
`Name`はソースを識別する為に使用される分類文字列である。 `Reward`is used to multiply karma associated with the source. A high `Reward` value means each point of karma generated by the source has more effect than a source with a lower `Reward`.
