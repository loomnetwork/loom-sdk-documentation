...
id: deploy-loom-mainnet title: Deploy to Loom Mainnet sidebar_label: Deploy to Loom Mainnet
...
Notice that, in order to deploy to Plasmachain, you need to pay 1400 LOOM. Click on the "Deposit more Loom to Plasmachain" and follow the instructions.
...
```js
<br />const LoomTruffleProvider = require('loom-truffle-provider')
const { sha256 } = require ('js-sha256')
const { CryptoUtils } = require ('loom-js')
const { mnemonicToSeedSync } = require ('bip39')
const fs = require('fs')
