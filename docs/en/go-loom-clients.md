...
To get started, install `go-loom` by running the following command:
...
In the `go-loom` package you will find a number of examples. `examples/cli` contains a CLI app that can be used to interact with the `examples/plugins/helloworld` smart contract. We'll start by building and test driving the CLI app, then we'll introduce you to the `go-loom` API that was used
to build it.
